...
export const CreateBoundNFTStepError: React.FC = () => {
  const { t } = useTranslation('pages')
  const { setScreenState, setIsBack, isBack, metaData, data } = useContext(BoundNFTContext)
  ...
  return (
    ...
    >
      ...
      <Box variant='frames.normal' sx={{ textAlign: 'center', mt: 40 }}>
        <Text as='span' variant='text.body' sx={{ color: 'text3', lineHeight: 1.5 }}>
          {t('paragraph.create-boundnft-error')}: {data?.errorMsg}
        </Text>
      </Box>
      ...
      >
        ...
        <Button text={t('button.back').toUpperCase()} outlined sx={{ mt: [20, 20, 20, 0] }} onClick={() => handleBack()} />
      </Flex>
    </MotionBox>
  )
}
