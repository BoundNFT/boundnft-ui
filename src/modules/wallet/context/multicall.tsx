...
// Interfaces
import { MulticallContextProps, MulticallStateProps, ReducerActionProps } from '../interfaces/multicall'
// Constants
import { Call } from '../interfaces/multicall'
import { useMulticallContract } from '../hooks/useContract'
import { parseCallKey, chunkArray, toCallKey } from '../utils/multicall'
...
export function activeListeningKeys(
  ...
  const listeners = allListeners?.[0]?.[chainId]
  if (showDebug) console.log('context activeListeningKeys chainId listeners', allListeners, JSON.stringify(listeners))
  if (!listeners) return {}
  ...
  const returnKeys = Object.keys(listeners).reduce<{
    ...
    memo[callKey] = keyListeners?.blocksPerFetch ?? 1
...
export function outdatedListeningKeys(
  ...
  return Object.keys(listeningKeys).filter(callKey => {
    ...
    if (showDebug) console.log('listeningKeys', listeningKeys)
    if (showDebug) console.log('listeningKeys[callKey]', listeningKeys[callKey])
    if (showDebug) console.log('blocksPerFetch', blocksPerFetch)
...
const MulticallContextProvider: React.FC = ({ children }) => {
  ...
  const { latestBlockNumber, network: { chainId }} = useWallet()
  ...
  const listeningKeys: { [callKey: string]: number } = useMemo(() => {
    ...
    return activeKeys
  }, [callListeners, chainId, debouncedListeners])
  ...
  useEffect(() => {
    ...
    if (showDebug) console.log('context multicall calls chainId fetchingBlockNumber', calls, chainId, latestBlockNumber)
...
*/
      ...
      cancellations: chunkedCalls.map((chunk, index) => {
        ...
        promise
          .then(({ results: returnData, blockNumber: fetchBlockNumber }) => {
            ...
            if (showDebug) console.log('context newCallResults', newCallResults?.[4])
            MulticallDispatch({
              type: 'UPDATE',
              payload: {
                callResults: newCallResults
  ...
  }, [chainId, multicallContract, MulticallDispatch, callResults, serializedOutdatedCallKeys, latestBlockNumber])
